...
function checkParentVisited(visited: Set<string>, parent: SearchNode): boolean {
  if (
    visited.has(parent[0].row.toString() + '_' + parent[0].column.toString())
  ) {
    return true;
  }
 ...
 */
function checkChildIsWall(
  walls: Set<string>,
  child: CoordinateAndDirection
): boolean {
...
function search(
  ...
  while (!frontier.isEmpty()) {
    ...
    if (popped) {
      const parent: SearchNode = isPriorityFrontier
        ? (popped as PriorityItem<SearchNode>).item
        : (popped as SearchNode);
      const parentPriority: number = isPriorityFrontier
        ? (popped as PriorityItem<SearchNode>).priority
        : 0;
      ...
      if (checkParentIsGoal(goal, parent)) {
        ...
        if (
          checkParentIsWall(walls, parent) ||
          checkParentVisited(visited, parent)
        ) {
          continue;
        } else {
          ...
          for (let i = 0; i < children.length; i++) {
            ...
            if (isPriorityFrontier && computePriority) {
              ...
              (frontier as PriorityQueueAscend<SearchNode>).push(
                [children[i], newPath],
                priority
              );
            } else {
            ...
            if (
              !checkChildVisited(visited, children[i]) &&
              !checkChildIsWall(walls, children[i]) &&
              !checkChildIsGoal(goal, children[i])
            ) {
          ...
          visited.add(
            parent[0].row.toString() + '_' + parent[0].column.toString()
          );
        }
      }
    }
...
function unweightedSearch(
  ...
  const frontier: UnweightedFrontier =
    algoType === 'BreadthFirstSearch'
...
function getWeight(
  ...
  coordinate: CoordinateAndDirection
): number {
...
function weightedSearch(
  ...
  if (algoType === 'GreedyBestFirstSearch') {
    ...
    computePriority = (child, _parentPriority, goal) =>
      getEuclideanDistance(child, goal);
    ...
    computePriority = (child, parentPriority, goal) =>
      parentPriority +
      getWeight(weights, child) +
      getEuclideanDistance(child, goal);
  } else {
    // Dijkstra's Algorithm: pure cost-so-far (terrain-weighted), no heuristic.
    computePriority = (child, parentPriority) =>
      parentPriority + getWeight(weights, child);
  }
  ...
  return search(
    ...
    scheduleTimeout,
    callbacks,
    new PriorityQueueAscend<SearchNode>(),
    computePriority,
    stepDelay
  );
}

export { unweightedSearch, weightedSearch };

...
const Configuration = () => {
	...
	const [boardSize, setBoardSize] = React.useState<{ rows: number; columns: number } | null>(null);
	const [startCoordinate, setStartCoordinate] = React.useState<CoordinateAndDirection | null>(null);
	const [goalCoordinate, setGoalCoordinate] = React.useState<CoordinateAndDirection | null>(null);
		...
		<div className="flex min-h-screen flex-col items-start gap-4 p-4 lg:flex-row lg:gap-6 lg:p-6">
			...
			<Card className="w-full shrink-0 gap-5 py-5 lg:w-[21rem]">
				<CardHeader className="gap-3 px-5">
					...
					{/* Legend */}
					<div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground" aria-label="Cell color legend">
				...
				<CardContent className="flex flex-col gap-5 px-5">
					...
					<div className="flex flex-col gap-2">
						...
						<Select value={wallAlgorithm} onValueChange={setWallAlgorithm}>
							<SelectTrigger id="wallAlgorithmChoices" aria-label="Wall Algorithm Choices">
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
								<SelectItem value="RecursiveDivision">Recursive Division</SelectItem>
								<SelectItem value="RecursiveDivisionTwoLayers">Twin Recursive Division</SelectItem>
					...
					<div className="flex flex-col gap-2">
						...
						<div role="group" aria-labelledby="paintModeLabel" className="inline-flex w-fit rounded-md border border-input bg-input/30 p-0.5">
							<Button
								type="button"
								size="sm"
					...
					<div className="flex flex-col gap-2">
						...
						<Select value={pathAlgorithm} onValueChange={setPathAlgorithm}>
							<SelectTrigger id="pathAlgorithmChoices" aria-label="Path Algorithm Choices">
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
								<SelectItem value="BreadthFirstSearch">Breadth-first Search</SelectItem>
								<SelectItem value="DepthFirstSearch">Depth-first Search</SelectItem>
								<SelectItem value="GreedyBestFirstSearch">Greedy Best-First Search</SelectItem>
								<SelectItem value="DijkstrasAlgorithm">Dijkstra's Algorithm</SelectItem>
					...
					{stats && (
						<div className="grid grid-cols-3 gap-2 rounded-md border border-border bg-muted/50 p-3 text-center" role="status">
							<div>
							...
							<div>
								<div className="text-lg font-bold">{stats.algorithmTimeMs >= 0 ? `${stats.algorithmTimeMs.toFixed(2)}ms` : "…"}</div>
					...
					<div className="flex flex-wrap gap-2">
						...
						<Button type="button" variant="outline" size="sm" onClick={() => { setErrorMessage(null); setStats(null); setShouldResetPath(true); }}>
							Reset Path
						</Button>
						<Button type="button" variant="outline" size="sm" onClick={() => { setErrorMessage(null); setStats(null); setShouldResetBoard(true); }}>
							Reset All
						</Button>
					</div>
				</CardContent>
			...
			<Card className="min-w-0 min-h-[28rem] flex-1 gap-0 self-stretch overflow-hidden py-0">
				...
				<CardContent className="min-h-0 flex-1 p-0">
					...
					<div className="h-full w-full bg-background/40 p-3">
						...
						<div ref={boardAreaRef} className="board-scroll h-full w-full overflow-auto">
